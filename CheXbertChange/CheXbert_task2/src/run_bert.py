...
def train(save_path, dataloaders, weights = np.ones(len(CONDITIONS)), f1_weights= None, model=None, device=None,
          optimizer=None, lr=LEARNING_RATE, log_every=LOG_EVERY,
          valid_niter=VALID_NITER, best_metric=0.0, num_epochs = NUM_EPOCHS):
     ...
     for epoch in range(num_epochs):
          ...
          for i, data in enumerate(train_ld, 0):
               ...
               optimizer.zero_grad()
               out = model(batch, attn_mask) #list of tensors
               batch_loss = 0.0
               ...
               if (i+1) % valid_niter == 0:
                    ...
                    metrics_train = utils.evaluate(model, train_ld, device)
     ...
     return(dev_loss,loss_values, accuracy_values, accuracy_values_train, F1_values, F1_values_train, loss_values_batch_level, loss_values_bis)
