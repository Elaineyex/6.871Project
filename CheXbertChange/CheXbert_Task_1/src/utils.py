...
def compute_positive_f1(y_true, y_pred):
    ...
    @param y_true (list): List of 2 tensors each of shape (dev_set_size)
    @param y_pred (list): Same as y_true but for model predictions 

    @returns res (list): List of 2 scalars
    """
...
def evaluate(model, dev_loader, device, f1_weights, return_pred=False):
    ...
    @returns res_dict (dictionary): dictionary with keys 'Positive', 'Accuracy', 'loss', with values 
                            being lists of length 2 with each element in the 
...
def test(model, checkpoint_path, test_ld, f1_weights):
    ...
    for j in range(len(CONDITIONS)):
        print('%s  positive_f1: %.3f' % (CONDITIONS[j], metrics['positive'][j]))
    for j in range(len(CONDITIONS)):
