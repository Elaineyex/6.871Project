...
def train(save_path, dataloaders, f1_weights, model=None, device=None,
     ...
     @param f1_weights (dictionary): maps conditions to weights for positive f1 task averaging
     ...
     loss_values = []
     loss_values_bis = []
     loss_values_batch_level = []
     accuracy_values = {k: [] for k in CONDITIONS}
     F1_values = {k: [] for k in CONDITIONS}
     dev_loss = []
     accuracy_values_train = {k: [] for k in CONDITIONS}
     F1_values_train = {k: [] for k in CONDITIONS}
     ...
     for epoch in range(NUM_EPOCHS):
          ...
          for i, data in enumerate(train_ld, 0):
               ...
               for j in range(len(out)):
                #for sigmoid
                    batch_loss += loss_func(out[j].squeeze().to(device), label[j].float().to(device))
                    
               report_loss += batch_loss
               report_examples += batch_size
               loss = batch_loss / batch_size     
               epoch_loss.append(loss.item())
               loss_values_batch_level.append(loss.item())
               loss.backward()
               optimizer.step()  
               ...
               if (i+1) % valid_niter == 0:
                    ...
                    metrics_train = utils.evaluate(model, train_ld, device, f1_weights)
                    
                    print("Evaluation on DEV set")
                    f1s = np.zeros(len(CONDITIONS))
                    for j in range(len(CONDITIONS)):
                         print('%s F1: %.3f' % (CONDITIONS[j], metrics['positive'][j]))
                         print('%s accuracy: %.3f' % (CONDITIONS[j], metrics['accuracy'][j]))
                         accuracy_values[CONDITIONS[j]].append(metrics['accuracy'][j])
                         F1_values[CONDITIONS[j]].append(metrics['positive'][j])
                         f1s[j] = metrics['positive'][j]
                    dev_loss.append(metrics['loss'])
                    metric_avg = np.mean(f1s)
                    print(f"Average F1 : {metric_avg}")
                                   
                    print("Evaluation on TRAIN set")
                    f1s_train = np.zeros(len(CONDITIONS))
                    for j in range(len(CONDITIONS)):
                         print('%s F1: %.3f' % (CONDITIONS[j], metrics_train['positive'][j]))
                         print('%s accuracy: %.3f' % (CONDITIONS[j], metrics_train['accuracy'][j]))
                         accuracy_values_train[CONDITIONS[j]].append(metrics_train['accuracy'][j])
                         F1_values_train[CONDITIONS[j]].append(metrics_train['positive'][j])
                         f1s_train[j] = metrics_train['positive'][j]
                    print(f"Average F1 : {np.mean(f1s_train)}")
                    loss_values_bis.append(metrics_train['loss'])
                    if metric_avg > best_metric: #new best network
                         ...
                         torch.save({'epoch': epoch+1,
                                    ...
                                    path)
          loss_values.append(np.mean(np.array(epoch_loss)))           
     return(dev_loss,loss_values, accuracy_values, accuracy_values_train, 
            F1_values, F1_values_train, loss_values_batch_level, loss_values_bis)
...
if __name__ == '__main__':
     parser = argparse.ArgumentParser(description='Train BERT-base model on task of labeling 2 change conditions.')
