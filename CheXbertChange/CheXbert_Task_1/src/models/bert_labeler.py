...
class bert_labeler(nn.Module):
    def __init__(self, p=0.1, clinical=False, freeze_embeddings=False, pretrain_path=None):
        ...
        self.linear_heads_bis = nn.ModuleList([nn.Linear(hidden_size, 1, bias=True) for _ in range(2)])
    ...
    def forward(self, source_padded, attention_mask):
        ...
        @returns out (List[torch.Tensor])): A list of size 2 containing tensors, having size (batch_size, 1)
        """
        ...
        out = []
        for i in range(2):
            out.append(self.linear_heads_bis[i](cls_hidden))
        return out
